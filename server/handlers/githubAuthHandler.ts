...
export async function handleGitHubLogin(req: Request, res: Response) {
  ...
  if (!CLIENT_ID) {
    ...
    return res
      .status(500)
      .send("Server configuration error: GitHub OAuth not configured");
  }
...
export async function handleSwitchInstallation(req: Request, res: Response) {
  try {
    ...
    if (!userId || !installationId) {
      return res
        .status(400)
        .json({ error: "userId and installationId required" });
    }
    ...
    if (!user.installationIds.includes(installationId)) {
      return res
        .status(403)
        .json({ error: "Access denied to this installation" });
    }
